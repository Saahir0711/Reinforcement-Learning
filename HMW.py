...
def reinforcement_learning_activity():
    ...
    prompt = input("Enter a prompt for the AI model (e.g., 'Descrive the lion'): ").strip()
    if not prompt:
    ...
    print("2. How does reinforcement learing help AI to improve its performance over time?")

def role_based_prompt_activity():
    print("\n===ROLE-BASED PROMPTS ACTIVITY===\n")
    category = input("Enter a category (e.g., science, history, maths): ").strip()
    ...
    buisiness_prompt = f"You are an business man of {category}. Explain {item} in a practical and application-focused manner."
    student_prompt = f"You are an student studying {category}. Explain {item} in a simple, informal manner."
    ...
    buisiness_resonse = generate_response(buisiness_prompt, temperature=0.3, max_tokens=1024)
    student_response = generate_response(student_prompt, temperature=0.3, max_tokens=1024)

    print(f"\n---Teacher's perspective---\n{teacher_response}")
    print(f"\n---Expert's Perspective---\n{expert_response}")
    print(f"\n---Buisiness Man's Perspective---\n{buisiness_resonse}")
    print(f"\n---Students's Perspective---\n{student_response}")

    print("\nReflection:")
    print("1. How did the AI's response differ between the teacher's, buisiness man's, student's and expert's perspectives?")
